...
<div class="courses-page">
    ...
    <style>
        /* ═══════════════════════════════════════════════
   PAGE LAYOUT + LEVEL COLOR SYSTEM
═══════════════════════════════════════════════ */
        .courses-page {
            display: flex;
            ...
            --level-beginner-soft: rgba(22, 163, 74, .12);
            --level-intermediate: #d97706;
            --level-intermediate-soft: rgba(217, 119, 6, .12);
            --level-advanced: #7c3aed;
            --level-advanced-soft: rgba(124, 58, 237, .12);
            --level-expert: #e11d48;
            --level-expert-soft: rgba(225, 29, 72, .12);
            --level-default: #64748b;
            --level-default-soft: rgba(100, 116, 139, .12);
        }
...
═══════════════════════════════════════════════ */
   ...
   Left: category overview + browse-all-courses.
   Right: sticky "My Enrolled Courses" sidebar.
═══════════════════════════════════════════════ */
        .courses-layout {
            display: grid;
        ...
        .courses-sidebar .section-card {
            display: flex;
            flex-direction: column;
            min-height: 0;
        }

        .courses-sidebar .section-card-body {
            overflow-y: auto;
        }
...
═══════════════════════════════════════════════ */
        ...
        .category-overview-card:hover .cov-arrow {
            color: var(--brand-primary);
...
═══════════════════════════════════════════════ */
        ...
        /* ═══════════════════════════════════════════════
   CATEGORY TABS (desktop/tablet) — scrollable, no dropdown swap needed
═══════════════════════════════════════════════ */
        .category-tabs-wrap {
            padding: 0 clamp(14px, 3vw, 24px) 0;
            border-bottom: 1.5px solid var(--line);
            display: flex;
            align-items: center;
            gap: 4px;
        ...
        .category-tabs-wrap::-webkit-scrollbar-thumb {
            background: var(--line);
            border-radius: 4px;
        }
        ...
        /* Mobile category dropdown (swaps in below 640px) */
...
═══════════════════════════════════════════════ */
        .level-snapshot-row {
            ...
            gap: 8px;
            flex-wrap: wrap;
            padding: 16px clamp(14px, 3vw, 24px) 0;
        }
...
═══════════════════════════════════════════════ */
        .subcategory-row {
            ...
            gap: 8px;
            flex-wrap: wrap;
            padding: 12px clamp(14px, 3vw, 24px) 0;
        }
...
═══════════════════════════════════════════════ */
        ...
        .level-section .course-grid {
            padding: 0 0 4px;
        }

        /* ═══════════════════════════════════════════════
   COURSE GRID — fully fluid, no hard breakpoint gaps
═══════════════════════════════════════════════ */
        .course-grid {
            display: grid;
            gap: 16px;
        ...
        /* Sidebar variant — single column list, more compact tiles */
        .course-list {
            display: flex;
            flex-direction: column;
            gap: 12px;
...
═══════════════════════════════════════════════ */
        ...
        /* Compact sidebar tile: thumbnail shrinks, no hover lift (keeps sidebar calm) */
        .course-tile.is-compact {
            flex-direction: row;
            align-items: stretch;
        }

        .course-tile.is-compact:hover {
            transform: none;
        }

        .course-tile.is-compact .course-thumb {
            width: 84px;
            flex-shrink: 0;
            aspect-ratio: auto;
            padding: 6px;
        ...
        .course-tile.is-compact .course-thumb h3 {
            display: none;
        }

        .course-tile.is-compact .course-body {
            padding: 10px 12px;
            gap: 6px;
        }

        .course-tile.is-compact .course-title-inline {
            font-size: 12.5px;
            font-weight: 700;
            color: var(--text);
            line-height: 1.3;
        ...
        .course-tile.is-compact .course-curriculum,
        .course-tile.is-compact .course-enrolled-badge {
            display: none;
        }

        .course-tile.is-compact .course-progress-wrap {
            gap: 3px;
        }
...
═══════════════════════════════════════════════ */
        ...
        .btn-unlock:hover:not(:disabled) {
            opacity: .88;
            transform: translateY(-1px);
            box-shadow: 0 5px 16px rgba(240, 179, 90, .4);
        }
...
═══════════════════════════════════════════════ */
        ...
        /* Large desktop: cap card growth so tiles don't get absurdly wide */
        ...
        /* Phone: stack tiles single-column below ~420px so text never crushes */
    ...
    @php
        ...
        // ASSUMPTION: Course::weeks() / Week::sessions() already exist.
        $curriculumOf = function ($course) {
            $weeks = $course->relationLoaded('weeks') ? $course->weeks : collect();
            $sessions = $weeks->sum(
                fn($w) => $w->relationLoaded('sessions') ? $w->sessions->count() : $w->sessions_count ?? 0,
            );
        ...
        // NOTE: $course->batches holds CourseBatch pivot rows (course_batch table).
        // The real Batch (seats, dates, mode) lives on the nested ->batch relation.
        // We filter on the *Batch's* status (upcoming/active), not the pivot's status.
        $openBatchesOf = fn($course) => $course->batches->filter(
            fn($cb) => $cb->batch && in_array($cb->batch->status ?? 'upcoming', ['upcoming', 'active']),
        );
    ...
    {{-- ══════════════════════════════════════════════════════════
     HERO HEADER (full width, above the two-column layout)
══════════════════════════════════════════════════════════ --}}
    <div class="courses-hero">
        <div class="courses-hero-left">
            <div class="courses-hero-icon" aria-hidden="true"><i class="ti ti-books"></i></div>
            <div>
                <div class="courses-hero-title">My Courses</div>
                <div class="courses-hero-sub">Browse every category and skill level — sorted low to high price so you
                    always see the best entry point first.</div>
            </div>
        </div>
    ...
    {{-- ══════════════════════════════════════════════════════════
     TWO-COLUMN LAYOUT
     Left column (.courses-main): category overview + browse-all-courses.
     Right column (.courses-sidebar): sticky "My Enrolled Courses".
══════════════════════════════════════════════════════════ --}}
    <div class="courses-layout">
        ...
        <div class="courses-main">
            ...
            <div class="section-card">
                <div class="section-card-head">
                    <div>
                        <div class="section-card-title"><i class="ti ti-layout-grid" aria-hidden="true"></i> Explore by
                            Category &amp; Level</div>
                        <div class="section-card-sub">See how many skill levels each category has, and their price
                            range, before you dive in.</div>
                    </div>
                    <span class="section-card-count">{{ $categoryOverview->count() }}
                        categor{{ $categoryOverview->count() === 1 ? 'y' : 'ies' }}</span>
                </div>
                ...
                <div class="category-overview-grid">
                    @foreach ($categoryOverview as $i => $row)
                        <button type="button" class="category-overview-card" data-jump="{{ $row->category->id }}"
                            wire:click="setActiveCategory({{ $row->category->id }})">
                            <div class="cov-top">
                                <div class="cov-icon"><i
                                        class="ti {{ $categoryIcons[$i % count($categoryIcons)] }}"></i></div>
                                <div>
                                    <div class="cov-title">{{ $row->category->name }}</div>
                                    <div class="cov-sub">{{ $row->count }} course{{ $row->count !== 1 ? 's' : '' }} ·
                                        {{ $row->levels->count() }} level{{ $row->levels->count() !== 1 ? 's' : '' }}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="cov-levels">
                                @foreach ($row->levels as $lvl)
                                    ...
                                    <span class="cov-level-chip"
                                        style="color:{{ $lc['fg'] }};background:{{ $lc['soft'] }};">{{ $lvl }}</span>
                                @endforeach
                            </div>
            ...
            <div class="section-card" id="browseAllCourses">
                <div class="section-card-head">
                    <div class="section-card-title"><i class="ti ti-layout-grid" aria-hidden="true"></i> Browse All
                        Courses</div>
                    <div class="filter-bar">
                        <span class="sort-note"><i class="ti ti-sort-ascending" aria-hidden="true"></i> Sorted: Low →
                            High Price</span>
                        <div class="filter-search">
                            <i class="ti ti-search" aria-hidden="true"></i>
                            <input type="search" id="courseSearch" placeholder="Search courses…" autocomplete="off"
                ...
                {{--
                Category tab bar (desktop/tablet). The active tab is now driven
                entirely by the server-side $activeCategoryId property via
                wire:click, instead of a client-only JS toggle. That fixes the
                bug where clicking a category appeared to do nothing: any other
                Livewire action (add to cart, pick batch, etc.) re-renders the
                component and morphs the DOM back to what Blade produced, which
                previously always meant "first category active" because nothing
                on the server remembered the click.
            --}}
                    ...
                    @foreach ($categories as $category)
                        @php
                            $catCount = $category->courses
                                ...
                                ->unique('id')
                                ->count();
                            $isActiveTab = (int) $activeCategoryId === (int) $category->id;
                        @endphp
                        <button class="cat-tab {{ $isActiveTab ? 'active' : '' }}" type="button" role="tab"
                            wire:click="setActiveCategory({{ $category->id }})" wire:key="cat-tab-{{ $category->id }}"
                ...
                <div class="category-dropdown-wrap">
                    <select id="categoryDropdown" class="filter-select" aria-label="Select category"
                        wire:change="setActiveCategory($event.target.value)">
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ (int) $activeCategoryId === (int) $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                @foreach ($categories as $category)
                    @php
                        ...
                        $levelGroups = $tabCourses
                            ...
                            ->sortBy(
                                fn($courses, $levelName) => $levelPriority[strtolower($levelName)] ?? 99,
                                SORT_REGULAR,
                                false,
                            );
                    @endphp

                    <div class="tab-panel {{ $isActiveTab ? 'active' : '' }}" id="panel-{{ $category->id }}"
                        role="tabpanel" data-tab-panel="{{ $category->id }}" wire:key="cat-panel-{{ $category->id }}">

                        @if ($levelGroups->count() > 0)
                            <div class="level-snapshot-row">
                                <span class="level-snapshot-label" aria-hidden="true">Levels:</span>
                                <button class="level-pill active" type="button" data-level="all"
                                ...
                                @foreach ($levelGroups as $levelName => $levelCourses)
                                    ...
                                    @endphp
                                    <button class="level-pill" type="button" data-level="{{ $slug }}"
                                        style="color:{{ $lc['fg'] }};">
                                        <span class="level-dot" style="background:{{ $lc['fg'] }};"></span>
                                        {{ $levelName }} · {{ $levelCourses->count() }} ·
                                        ₹{{ number_format($levelCourses->min('price')) }}–₹{{ number_format($levelCourses->max('price')) }}
                                    </button>
                                @endforeach
                            </div>
                        @endif
                        ...
                        @if ($category->children->isNotEmpty())
                            <div class="subcategory-row">
                                ...
                                @foreach ($category->children as $child)
                                    <button class="sub-pill" type="button"
                        ...
                        @forelse ($levelGroups as $levelName => $levelCourses)
                            ...
                            <div class="level-section" data-level-section="{{ $slug }}">
                                <div class="level-section-head">
                                    <span class="level-dot" style="background:{{ $lc['fg'] }};"></span>
                                    <h4>{{ $levelName }}</h4>
                                    <span class="level-section-count">{{ $sortedCourses->count() }}
                                        course{{ $sortedCourses->count() !== 1 ? 's' : '' }}</span>
                                    <span class="level-section-price">
                                        <i class="ti ti-sort-ascending" aria-hidden="true"></i>
                                        ₹{{ number_format($sortedCourses->min('price')) }} –
                                        ₹{{ number_format($sortedCourses->max('price')) }}
                                    </span>
                                </div>
                                ...
                                <div class="course-grid">
                                    @foreach ($sortedCourses as $course)
                                        @php
                                            ...
                                            $bg = $thumb
                                                ? "url('{$thumb}')"
                                                : 'linear-gradient(135deg, #0947a8 0%, #7a5cff 100%)';
                                            ...
                                            $subCatId = $course->subcategory?->id
                                                ? (string) $course->subcategory->id
                                                : 'none';
                                            $price = $course->price ?? 0;
                                            $original = $course->original_price ?? null;
                                        ...
                                        @if ($enrolled)
                                            {{-- ENROLLED TILE (still shown here too, so "browse" stays a complete catalog view) --}}
                                            <a class="course-tile"
                                                href="{{ route('student.courses.show', $course) }}"
                                                data-subcat="{{ $subCatId }}" data-level="{{ $slug }}"
                                                ...
                                                <div class="course-thumb"
                                                    ...
                                                    <span class="course-enrolled-badge"><i class="ti ti-check"
                                                            aria-hidden="true"></i> Enrolled</span>
                                                    <h3>{{ $course->title }}</h3>
                                                </div>
                                                ...
                                                <div class="course-body">
                                                    <div class="course-meta">
                                                        <span class="course-level-tag"
                                                            style="color:{{ $lc['fg'] }};background:{{ $lc['soft'] }};">{{ $levelName }}</span>
                                                        <span class="course-type-tag"><i class="ti ti-category"
                                                                style="font-size:11px"></i> {{ $type }}</span>
                                                        <span class="course-meta-dot" aria-hidden="true"></span>
                                                        <i class="ti ti-folder" style="font-size:13px"
                                                            aria-hidden="true"></i>
                                                        {{ $catLabel }}
                                                    </div>
                                                    <div class="course-curriculum">
                                                        <span><i class="ti ti-calendar-week"></i> {{ $curr['weeks'] }}
                                                            wk</span>
                                                        <span><i class="ti ti-playlist"></i> {{ $curr['sessions'] }}
                                                            sessions</span>
                                                    </div>
                                                    <div class="course-foot">
                                                        <span class="btn btn-success btn-sm"><i
                                                                class="ti ti-player-play" aria-hidden="true"></i> Open
                                                            Course</span>
                                                    </div>
                                                </div>
                                            </a>
                                        @else
                                            ...
                                            <div class="course-tile is-locked"
                                                ...
                                                data-subcat="{{ $subCatId }}" data-level="{{ $slug }}"
                                                data-title="{{ strtolower($course->title) }}" tabindex="0"
                                                role="article"
                                                aria-label="{{ $course->title }} — locked, price {{ $priceLabel }}, level {{ $levelName }}">

                                                <a href="{{ route('student.courses.preview', $course->id) }}"
                                                    style="text-decoration:none;color:inherit;">
                                                    <div class="course-thumb"
                                                        style="background-image: {{ $bg }};">
                                                        <div class="course-thumb-overlay" aria-hidden="true"></div>
                                                        <div class="course-lock-overlay" aria-hidden="true">
                                                            <div class="course-lock-icon"><i class="ti ti-lock"></i>
                                                            </div>
                                                        </div>
                                                        <span class="course-level-badge"
                                                            style="background:{{ $lc['fg'] }};">{{ $levelName }}</span>
                                                        @if ($inCart)
                                                            <span class="course-cart-badge"><i
                                                                    class="ti ti-shopping-cart"></i> In Cart</span>
                                                        @endif
                                                        <h3>{{ $course->title }}</h3>
                                                    </div>
                                                </a>
                                                ...
                                                <div class="course-body">
                                                    ...
                                                    <div class="course-curriculum">
                                                        <span><i class="ti ti-calendar-week"></i> {{ $curr['weeks'] }}
                                                            week{{ $curr['weeks'] !== 1 ? 's' : '' }}</span>
                                                        <span><i class="ti ti-playlist"></i> {{ $curr['sessions'] }}
                                                            session{{ $curr['sessions'] !== 1 ? 's' : '' }}</span>
                                                    ...
                                                    @if ($openBatches->isNotEmpty())
                                                        <div class="batch-picker">
                                                            ...
                                                            @foreach ($openBatches as $courseBatch)
                                                                @php
                                                                    ...
                                                                    $isLow =
                                                                        !$isFull &&
                                                                        $max > 0 &&
                                                                        $left <= max(1, (int) round($max * 0.15));
                                                                    $isActive =
                                                                        (int) $selectedBatchId ===
                                                                        (int) $courseBatch->id;
                                                                @endphp
                                                                <button type="button"
                                                                    class="batch-option {{ $isActive ? 'active' : '' }} {{ $isFull ? 'is-full' : '' }}"
                                                                    wire:click="pickBatch({{ $course->id }}, {{ $courseBatch->id }})"
                                                                    @disabled($isFull)
                                                                    aria-pressed="{{ $isActive ? 'true' : 'false' }}">
                                                                    <span class="batch-option-left">
                                                                        <span class="batch-option-code">
                                                                            {{ $batch->batch_code }}
                                                                            <span
                                                                                class="batch-mode-chip">{{ ucfirst($batch->mode ?? 'Online') }}</span>
                                                                        </span>
                                                                        <span class="batch-option-date">
                                                                            @if ($batch->start_date)
                                                                                Starts
                                                                                {{ \Illuminate\Support\Carbon::parse($batch->start_date)->format('d M Y') }}
                                                                            @else
                                                                                Start date TBA
                                                                            @endif
                                                                        </span>
                                                                    ...
                                                                    <span class="seats-info">
                                                                        <span
                                                                            class="seats-count {{ $isFull ? 'is-full' : ($isLow ? 'is-low' : '') }}">
                                                                            {{ $isFull ? 'Full' : $left . ' seat' . ($left !== 1 ? 's' : '') . ' left' }}
                                                                        </span>
                                                                        <span class="seats-track">
                                                                            <span
                                                                                class="seats-fill {{ $isFull ? 'is-full' : ($isLow ? 'is-low' : '') }}"
                                                    ...
                                                    @elseif ($course->batches->isEmpty())
                                                        <div class="self-paced-note">
                                                            ...
                                                            Self-paced — start anytime, no batch needed
                                                        </div>
                                                    @else
                                                        <div class="self-paced-note">
                                                        ...
                                                        <span class="price-tag">
                                                            @if ($price > 0)
                                                                ...
                                                                @if ($original && $original > $price)
                                                                    <span
                                                                        class="price-old">₹{{ number_format($original) }}</span>
                                                                @endif
                                                                @if ($course->gst)
                                                                    <span class="price-gst">+{{ $course->gst }}%
                                                                        GST</span>
                                                                @endif
                                                            @else
                                                        ...
                                                        <div style="display:flex; gap:6px;">
                                                            @if ($inCart)
                                                                <button type="button" class="btn btn-outline btn-sm"
                                                                    wire:click="removeFromCart({{ $course->id }})">
                                                                    <i class="ti ti-shopping-cart-x"
                                                            ...
                                                            @endif
                                                            <button type="button" class="btn btn-unlock btn-sm"
                                                                wire:click="buyNow({{ $course->id }})">
                                                                <i class="ti ti-bolt" aria-hidden="true"></i> Buy Now
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                ...
                @error('cart')
                    <div style="padding: 0 24px 20px;"><span class="error-text">{{ $message }}</span></div>
                @enderror
            </div>
        ...
        {{-- ══════════════════════════════════════════════════════════
         SIDEBAR — My Enrolled Courses (sticky, right column)
    ══════════════════════════════════════════════════════════ --}}
        <div class="courses-sidebar">
            <div class="section-card">
                <div class="section-card-head">
                    <div class="section-card-title"><i class="ti ti-star" aria-hidden="true"></i> My Enrolled Courses
                    </div>
                    <span class="section-card-count">{{ $enrolledCourses->count() }}
                        course{{ $enrolledCourses->count() !== 1 ? 's' : '' }}</span>
                </div>
                ...
                <div class="section-card-body">
                    <div class="course-list">
                        @forelse ($enrolledCourses as $course)
                            ...
                            <a class="course-tile is-compact" href="{{ route('student.courses.show', $course) }}"
                                aria-label="Open {{ $course->title }}">
                                ...
                                <div class="course-body">
                                    <div class="course-title-inline">{{ $course->title }}</div>
                                    <span class="course-level-tag"
                                        style="color:{{ $lc['fg'] }};background:{{ $lc['soft'] }};width:fit-content;">{{ $course->level ?? 'Beginner' }}</span>
                                    ...
                                    @if ($progress > 0)
                                        <div class="course-progress-wrap">
                                            ...
                                            <div class="course-progress-track">
                                                <div class="course-progress-bar" style="width: {{ $progress }}%"
                        ...
                        @empty
                            <div class="empty-state">
                                ...
                                <p>You haven't enrolled in any courses yet.<br>Browse on the left to find your first
                                    one!</p>
                            </div>
                        @endforelse
                    </div>
...
══════════════════════════════════════════════════════════ --}}
    ...
    <div class="cart-drawer" id="amCartDrawer">
        <div class="cart-drawer-head">
            ...
            <button class="modal-close" onclick="closeAmCart()" aria-label="Close cart"><i
        ...
        <div class="cart-drawer-body">
            @forelse ($this->cartCourses as $course)
                ...
                @endphp
                <div class="cart-item">
                    <div class="cart-item-thumb" style="background-image: {{ $bg }};"></div>
                    <div class="cart-item-info">
                        <div class="cart-item-name">{{ $course->title }}</div>
                        <div class="cart-item-price">
                            {{ $course->price > 0 ? '₹' . number_format($course->price) : 'Free' }}</div>
                        @if ($courseBatch?->batch)
                            <div class="cart-item-batch">{{ $courseBatch->batch->batch_code }} ·
                                {{ ucfirst($courseBatch->batch->mode ?? 'Online') }}</div>
                        @endif
                    </div>
                    <button type="button" class="cart-item-remove"
                        wire:click="removeFromCart({{ $course->id }})" aria-label="Remove {{ $course->title }}">
        ...
        @if (count($cartIds) > 0)
            <div class="cart-drawer-foot">
                ...
                <button type="button" class="btn btn-primary btn-block" wire:click="checkout">
                    <i class="ti ti-lock" aria-hidden="true"></i> Checkout Securely
                </button>
                @error('payment')
        ...
        <script>
            ...
            function amInitTabs() {
                document.querySelectorAll('.cat-tab').forEach(btn => {
                    btn.addEventListener('click', () => {
                        ...
                        if (activePanel) {
                            activePanel.querySelectorAll('.sub-pill').forEach(p => p.classList.toggle('active',
                                p.dataset.subtab === 'all'));
                            activePanel.querySelectorAll('.level-pill').forEach(p => p.classList.toggle(
                                'active', p.dataset.level === 'all'));
                ...
                document.querySelectorAll('.tab-panel').forEach(panel => {
                    panel.querySelectorAll('.sub-pill').forEach(pill => {
                        pill.addEventListener('click', () => {
                            panel.querySelectorAll('.sub-pill').forEach(p => p.classList.remove(
                                'active'));
                    ...
                    panel.querySelectorAll('.level-pill').forEach(pill => {
                        pill.addEventListener('click', () => {
                            panel.querySelectorAll('.level-pill').forEach(p => p.classList.remove(
                                'active'));
                            pill.classList.add('active');
                            amApplyFilters(panel);
                        });
            ...
            function amInitSearch() {
                ...
                input.addEventListener('input', function() {
                    document.querySelectorAll('.tab-panel').forEach(panel => {
                        panel.querySelectorAll('.sub-pill').forEach(p => p.classList.toggle('active', p.dataset
                            .subtab === 'all'));
                        panel.querySelectorAll('.level-pill').forEach(p => p.classList.toggle('active', p
                            .dataset.level === 'all'));
                        amApplyFilters(panel);
                    });
                });
        ...
        <script>
            ...
            document.addEventListener('livewire:init', () => {
                Livewire.on('show-alert', (event) => {
                    Swal.fire({
                        icon: event[0].type,
                        title: event[0].type === 'error' ? 'Error' : 'Success',
